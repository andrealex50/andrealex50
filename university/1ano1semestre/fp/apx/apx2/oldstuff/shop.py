...
def loadDataBase(fname, produtos):
    ...
    with open(fname,'r', encoding="utf-8") as file:
        for line in file:
            produtos = line.strip().split(';')
            codigo = produtos[0]
            descricao_produto = tuple(produtos[1:5])
            if codigo not in d:
                d[codigo] = descricao_produto
        return d
...
def registaCompra(produtos):
    ...
    """
    unidades = {}
    loadDataBase(fname, produtos)
    while True:
        code = input('Code? ')
        if code not in d.keys() and code != '':
            code = input('Code? ')
        elif code == '':
            break
        else:
...
def fatura(produtos, compra):
    ...
    for produto in listaprodutos:
        ...
        seccao = produto[1] 
        taxa = produto[3]
        taxa_for_calculus = float(taxa.strip('%'))
...
def main(args):
    ...
    while repetir:
        ...
        if op == "F":
            numero = int(input('Número compra? '))
